...
interface Props { drop: Drop; index: number }

export default function LiveCard({ drop, index }: Props) {
  const delayClass = DELAY_CLASSES[(index % 4) + 1]
  return (
    <Link to={"/artwork/" + drop.id} className={"art-card group reveal " + delayClass + " flex flex-col"} style={{ textDecoration: 'none' }}>
      <div className="relative overflow-hidden" style={{ aspectRatio: '1 / 1' }}>
        <img src={drop.image} alt={drop.title} className="w-full h-full object-cover transition-transform duration-700 ease-out group-hover:scale-[1.04]" loading="lazy" />
        ...
        <div className="absolute top-4 left-4 flex items-center gap-2 px-3 py-1.5" style={{ background: 'rgba(10,10,10,0.85)', backdropFilter: 'blur(8px)' }}>
          ...
          <span className="font-label" style={{ fontSize: '9px', color: '#FF3B30' }}>Live</span>
        </div>
      ...
      <div className="flex flex-col gap-4 p-5" style={{ background: 'var(--ink-2)' }}>
        <div>
          <p className="font-label mb-1.5" style={{ color: 'var(--sub)', fontSize: '9px' }}>{drop.artist}  {drop.album}</p>

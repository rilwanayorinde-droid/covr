export interface Drop {
  id: number; title: string; artist: string; album: string; image: string
  currentBid: string; topBidder: string; endsIn: { h: number; m: number; s: number }
  edition: string; live: boolean; bids: number; genre: string
  artistType: 'independent' | 'label'; reservePrice: string; story: string; spotifyUrl?: string
}
export interface UpcomingDrop {
  id: number; title: string; artist: string; image: string; dropsIn: string
  daysAway: number; edition: string; genre: string; artistType: 'independent' | 'label'; releaseDate: string
}
export interface Artist {
  id: number; name: string; genre: string; pieces: number; sales: string; avatar: string
  verified: boolean; artistType: 'independent' | 'label'; followers: string
  monthlyListeners: string; bio: string; instagram: string; drops: number
}
export interface Collector {
  id: number; name: string; avatar: string; totalSpent: string; pieces: number; rank: number
}

export const LIVE_DROPS: Drop[] = [
  { id: 1, title: 'Invisible Cities', artist: 'SOLIS', album: 'Meridian', image: 'https://images.unsplash.com/photo-1508700115892-45ecd05ae2ad?w=800&q=90', currentBid: '₦520,000', topBidder: 'collector_44', endsIn: { h: 3, m: 41, s: 22 }, edition: '1 of 1', live: true, bids: 14, genre: 'Alternative R&B', artistType: 'independent', reservePrice: '₦200,000', story: 'Created over three sleepless nights during a power outage in Lagos. The darkness became the palette.', spotifyUrl: 'https://spotify.com' },
...
export const FEATURED_ARTISTS: Artist[] = [
  ...
  { id: 3, name: 'Kenn Ola', genre: 'Afropiano', pieces: 22, sales: '₦6.1M', avatar: 'https://images.unsplash.com/photo-1560250097-0b93528c311a?w=500&q=90', verified: true, artistType: 'label', followers: '280K', monthlyListeners: '890K', bio: "One of Afropiano's most consistent voices. Kenn Ola's artwork is produced in collaboration with emerging visual artists across West Africa.", instagram: '@kennola', drops: 22 },
]
